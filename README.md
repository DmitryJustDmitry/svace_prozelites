...
*0. Корректно подготовленный тест состоит из:*
- исходных текстов теста
- описания порядка и параметров сборки теста
- подробное описание проблемы и ожидаемого от svace результата
- при необходимости явного указания - опций настройки среды сборки теста (в качестве таковых могут выступать процессорная архитектура, тулчейн сборки, дополнительно необходимые в системе библиотеки, переменные среды окружения и т.п.)
1. Старайтесь минимизировать кодовую базу теста и параметры компиляции теста таким образом, чтобы минимизировать время компиляции теста *(в том числе за счет изменения параметров конфигурации и компиляции анализируемого проекта - например исключением из процесса сборки всей кодовой базы, не являющейся необходимой для компиляции собственно кода, составляющего тест)*. 
*2. Рекомендуется (это рекомендация, а не требование) стремиться к тому, чтобы тест собирался просто вызовом компилятора с указанием файла исходных текстов (единицы трансляции), без задействования систем автоматизации сборки, таких как `make`.*
3. В случае, если вы не уверены в том, что интересующий вас участок кода действительно попадает в финальную компиляцию (например вы подозреваете, что компилятор может его соптимировать или удалить полностью - *типовой причиной является сборка проекта в режиме глубокой оптимизации: ключ компилятора `-O3`*), рекомендуется встроить в исходный текст теста в непосредственной близости к интересующему участку гарантированно обнаруживаемую svace проблему. Например таковой может выступать [деление на ноль](README.md#контроль-того-что-svace-действительно-проанализировал-ваш-код). 
4. В исходный код теста *желательно (это рекомендация, а не требование)* не закладывать каких-либо ошибок, выявляемых svace, за исключением той конкретной ошибки, которую вы закладываете в данный тест. 
5. В целях обеспечения сборки в окружении, отличном от созданного на вашем хоста, рекомендуется *(это рекомендация, а не требование)* оформлять тест в формате контейнера, для чего следует
...
6. Все найденные сущности (пакеты, классы и методы; их слабые параметры; описание известных уязвимостей; переиспользованные примеры кода и т.п.) рекомендуется приводить с ссылками на источники их исходного расположения в Интернет.
...
**Сперва всегда стоит поискать какой-то готовый тест, который использует интересующий нас класс, и дополнительно модицифировать его.**
...
### Контроль того, что svace действительно проанализировал ваш код
